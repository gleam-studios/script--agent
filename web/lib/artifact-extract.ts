...
export function looksLikeTemplateDeliverable(content: string): boolean {
  ...
  if (/(?:^|\n)##\s*第[一二三]幕/m.test(c)) return true;
...
function extractStage1(content: string): Artifact[] {
  ...
  if (oneliner) {
    results.push({ stage: 1, subKey: "oneliner", label: "一句话梗概", content: oneliner.body, updatedAt: now() });
  }

  const detailSynopsis = sections.find((s) => /详细剧情梗概/.test(s.heading));
  const outlineSections = sections.filter(
    (s) => !/一句话梗概/.test(s.heading) && !/详细剧情梗概/.test(s.heading)
  );
  ...
  if (results.length === 0) {
    /** 兼容 **一句话梗概**、【一句话梗概】、破折号、无加粗等常见模型写法 */
    const onelinerPatterns: RegExp[] = [
      /(?:^|\n)\s*(?:#{1,3}\s*|\*{0,2}\s*)?一句话梗概(?:\s*\*{0,2})?\s*[：:：\-—]\s*([^\n]+)/u,
      /(?:^|\n)\s*【\s*一句话梗概\s*】\s*[：:：\-—]?\s*([^\n]+)/u,
      /(?:^|\n)\s*「\s*一句话梗概\s*」\s*[：:：\-—]?\s*([^\n]+)/u,
    ];
    for (const re of onelinerPatterns) {
...
function normalizeStage5Markdown(c: string): string {
  let s = c.replace(/\ufeff/g, "");
  s = s.replace(/([^\s\r\n#])(##\s+)/g, "$1\n$2");
...
function extractStage5(content: string): Artifact[] {
  const results: Artifact[] = [];
  const topSections = splitBySections(normalizeStage5Markdown(content), "##");
...
export function reExtractForPreferredStage(
  ...
  if (!trimmed || preferredStage < 1 || preferredStage > 5) return null;
